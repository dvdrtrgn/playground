...
(function () {
  ...
  function d3_voronoi_tessellate(vertices, callback) {
    ...
    while (true) {
      ...
      if (newSite && (EventQueue.empty() || newSite.y < newIntStar.y || (newSite.y == newIntStar.y && newSite.x <
          newIntStar.x))) {
...
(function () {
  ...
  d3.layout.treemap = function () {
    var hierarchy = d3.layout.hierarchy(),
      ...
      sticky = false,
      stickies, ratio = 0.5 * (1 + Math.sqrt(5));
    ...
    function squarify(node) {
      ...
      if (children && children.length) {
        var rect = pad(node),
          ...
          child, best = Infinity,
          score, u = Math.min(rect.dx, rect.dy),
          n;
    ...
    treemap.padding = function (x) {
      ...
      function padFunction(node) {
        ...
        return p == null ? d3_layout_treemapPadNull(node) : d3_layout_treemapPad(node, typeof p === 'number' ? [p,
          p, p, p] : p);
      }
      ...
      var type;
      pad = (padding = x) == null ? d3_layout_treemapPadNull : (type = typeof x) === 'function' ? padFunction :
        type === 'number' ? (x = [x, x, x, x], padConstant) : padConstant;
      return treemap;
    };
