...
vis = div.append('svg').attr({
  width: width + padding * 2,
  height: height + padding * 2,
}).append('g').attr({
  transform: 'translate(' + [radius + padding, radius + padding] + ')',
});
...
(function (json) {
  ...
  path.enter().append('path').attr({
    ...
    d: arc,
    'fill-rule': 'evenodd',
  }).style('fill', colour).on('click', click);

  text = vis.selectAll('text').data(nodes);

  textEnter = text.enter().append('text').style('fill-opacity', 1).style('fill', function (d) {
